...
void AddConfigs(const string cfgfnm, NonLinearRegression& dtr, int proc_rank, int proc_size)
{
	...
	for (int cntr = 0; cfg.Load(ifs); cntr++)
	{
		if (cntr % proc_size == proc_rank)
		{
			...
			//std::cout << cfg.type_num[0]<< "  " << cfg.type_num[1] << std::endl;
		}
...
void Train_MTPR(std::vector<std::string>& args, std::map<std::string, std::string>& opts)
{
	...
	for (int i = 0; i < end; i++)
	{
       ...
       if (prank == 0) { for (int i =0; i<mtpr.mu_.size();i++)
						 {
							std::cout << mtpr.mu_[i]<< "  " ;
						 }
						 std::cout << std::endl;    }
	...
	if (opts["init-params"] == "random" && !mtpr.inited) {
		if (prank == 0) {
			...
			for (int k = 0; k < mtpr.species_count; k++)
				for (int l = k; l < mtpr.species_count; l++)
					for (int i = 0; i < mtpr.radial_func_count; i++) {
						for (int j = 0; j < rb_size; j++)
							mtpr.regression_coeffs[(k * mtpr.species_count + l) * mtpr.radial_func_count * rb_size +
							i * rb_size + j]
							= mtpr.regression_coeffs[(l * mtpr.species_count + k) * mtpr.radial_func_count * rb_size +
							i * rb_size + j]
