...

	{
		...
		for (int i = 0; i < alpha_index_basic_count; i++)
			for (int j = 0; j < C; j++)
				for (int k = 0; k < R*K; k++)
				{
					out_grad_accumulator[(C*type_central + j)*R*K + k] += 1*dloss_dmom[i] * mom_rad_jacobian_(i, j, k); //from basic moments
					...
					out_grad_accumulator[(C*type_central + j)*R*K + k] += 1*site_energy_ders_wrt_moments_[i] * mom_rad_coord_jacobian_(i, j, k); //from basic moment ders 
